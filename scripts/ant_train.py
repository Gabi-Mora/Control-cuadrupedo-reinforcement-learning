...
train_env = make_vec_env('Ant-v5', n_envs=n_training_envs, seed=0)
train_env = VecMonitor(train_env, logdir)
eval_env = make_vec_env('Ant-v5', n_envs=n_eval_envs, seed=0)

model = PPO("MlpPolicy", train_env, verbose=1, tensorboard_log=logdir)

callback = CheckPointCallback(check_freq=2048, log_dir=logdir)

eval_callback = EvalCallback(eval_env, best_model_save_path=models_dir + '/best_model/',
                              log_path=models_dir + '/best_model/', eval_freq=max(500 // n_training_envs, 1),
